...
class MyHandler(BaseHTTPRequestHandler):
    ...
    def _interactive_running(self, opt, reply_text):
        ...
        f.write("Question : " + reply_text + "\n")
        a=interpreter.parse(reply_text)
        b=a['intent']['name']
        c = a['intent']['confidence']
        if c>=0.70:
            text=df.loc[b].values[0]
            if type(text) != str:
                text = text[0]
            model_res ={'id': 'TransformerGenerator', 'episode_done': False, 'text': text}
            f.write("Bot Answer : " + model_res['text'] + "\n")
            f.close()
            return model_res
        else:
